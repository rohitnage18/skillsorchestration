...
export async function GET(req: Request) {
  try {
    ...
    return errorResponse(error, "Unable to list workflows.", getRouteErrorStatus(error));
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return errorResponse(error, "Unable to create workflow.", getRouteErrorStatus(error));
  }
}
